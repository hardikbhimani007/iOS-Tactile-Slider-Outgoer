...
@IBDesignable open class TactileSlider: UIControl {
    ...
    /// When set to at least two colors, the filled portion of the track uses a linear gradient between the first two colors instead of a solid ``tintColor``.
    ///
    /// The gradient runs along the value axis (horizontal or vertical according to ``vertical`` and ``reverseValueAxis``). The second gradient stop follows the normalized ``value`` between ``minimum`` and ``maximum``, so the blend updates as the user drags.
    ///
    /// Set to `nil` (default) to use a solid ``tintColor`` for the thumb. For outline contrast, consider setting ``tintColor`` to the first gradient color when using this property.
    open var gradientTintColors: [UIColor]? = nil {
        didSet {
            renderer.gradientTintColors = gradientTintColors
        }
    }
    ...
    open override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        renderer.trackBackground = trackBackground
        renderer.updateResolvedGradientColors()
        renderer.updateOutlineColors()
        tintColorDidChange()
    }
